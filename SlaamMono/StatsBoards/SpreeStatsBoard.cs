...
namespace SlaamMono.StatsBoards
{
    class SpreeStatsBoard : StatsBoard
    {
        ...
        public SpreeStatsBoard(MatchScoreCollection scorekeeper, Rectangle rect, Color col, IResources resources, IRenderGraph renderGraph)
            : base(scorekeeper)
        {
            SpreeStatsPage = new SpreePlayerStatsPageListing[scorekeeper.ParentGameScreen.x_ToRemove__Characters.Count];
            MainBoard = new Graph(rect, 2, col, resources, renderGraph);
        }

        public override void CalculateStats()
        {
            int[] TotalScore = new int[ParentScoreCollector.ParentGameScreen.x_ToRemove__Characters.Count];
