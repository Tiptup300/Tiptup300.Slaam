...
namespace Slaam
{
    class BoardThumbnailViewer : Screen
    {
        ...
        public void Update()
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (Input.Players[0].PressingDown)
                        Vertical = Direction.Down;
                    if (Input.Players[0].PressingUp)
                        Vertical = Direction.Up;

                    if (Input.Players[0].PressingRight)
                        Horizontal = Direction.Right;
                    if (Input.Players[0].PressingLeft)
                    ...
                    if (HOffset == 0 && VOffset == 0 && Input.Players[0].PressedAction)
                    {
                        Chosen = true;
                    }
        ...
        public void Draw(SpriteBatch batch)
        {
                ...
                for (int y = 0; y < 11; y++)
                {
                    ...
                    if (!(Pos.X < -DrawSizeWidth || Pos.X > GameGlobals.DRAWING_GAME_WIDTH || Pos.Y < -DrawSizeHeight || Pos.Y > GameGlobals.DRAWING_GAME_HEIGHT))
                    {
                        if (DrawingBoardIndex.Value < Boards.Count && Boards[DrawingBoardIndex.Value] != null)
                            ...
                            batch.Draw(Resources.NowLoading.Texture, Pos, Color.White);
                    }
            ...
            if (!StillLoadingBoards)
            {
                ...
                batch.Draw(Resources.BoardSelect.Texture, CenteredRectangle, new Color((byte)255, (byte)255, (byte)255, (byte)Alpha));
#if !ZUNE
        ...
        private void ContinueLoadingBoards()
        {
            //if (boards[CurrentBoardLoading].EndsWith(".png"))
            {
                try
                {
                    Texture2D temp = Game1.Content.Load<Texture2D>("content\\Boards\\" + GameGlobals.TEXTURE_FILE_PATH + boards[CurrentBoardLoading]);//Texture2D.FromFile(Game1.Graphics.GraphicsDevice, boards[CurrentBoardLoading]);
