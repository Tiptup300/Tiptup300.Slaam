...
namespace Slaam
{
    public class GameScreen : Screen
    {
        ...
        public Vector2 FinalBoardPosition
        {
            get
            {
                int width = (int)(Game1.Graphics.PreferredBackBufferWidth/2f);
                int height = (int)(Game1.Graphics.PreferredBackBufferHeight / 2f);
        ...
        public GameScreen(List<CharacterShell> chars)
        {
            SetupChars = chars;
        ...
        {
            //Characters.Add(new Character(Texture2D.FromFile(Game1.Graphics.GraphicsDevice, "skins\\Tiptup300_link.png"), ProfileManager.Profiles[0], new Vector2(800, 800), Input.Players[0], Color.Red));
            //Characters.Add(new Character(Texture2D.FromFile(Game1.Graphics.GraphicsDevice, "skins\\link901_Toadstool.png"), ProfileManager.Profiles[0], new Vector2(300, 300), Input.Players[1], Color.Black));
        ...
        {
            Tileset = Game1.Content.Load<Texture2D>("content\\Boards\\" + GameGlobals.TEXTURE_FILE_PATH + BoardLoc);//Texture2D.FromFile(Game1.Graphics.GraphicsDevice, BoardLoc);

            for (int x = 0; x < SetupChars.Count; x++)
            {
                if (SetupChars[x].Type == PlayerType.Player)
                {
                    Characters.Add(new Character(Game1.Content.Load<Texture2D>("content\\skins\\" + SetupChars[x].SkinLocation) /*Texture2D.FromFile(Game1.Graphics.GraphicsDevice, SetupChars[x].SkinLocation)*/, SetupChars[x].CharProfile, new Vector2(-100, -100), Input.Players[(int)SetupChars[x].PlayerIDX], SetupChars[x].PlayerColor, x));
                }
                else
                {
                    ProfileManager.AllProfiles[SetupChars[x].CharProfile].Skin = SetupChars[x].SkinLocation;
                    Characters.Add(new BotPlayer(Game1.Content.Load<Texture2D>("content\\skins\\" + SetupChars[x].SkinLocation) /*Texture2D.FromFile(Game1.Graphics.GraphicsDevice, SetupChars[x].SkinLocation)*/, SetupChars[x].CharProfile, new Vector2(-100, -100), this, SetupChars[x].PlayerColor, Characters.Count));
                }
#if !ZUNE
        ...
        public virtual void Update()
        {
            ...
            {
#if !ZUNE
                if (Input.Players[0].PressedUp || Input.Players[0].PressedDown)
                    PauseChoice = !PauseChoice;

                if (PauseChoice && Input.Players[0].PressedAction)
                {
                    BackgroundManager.ChangeBG(BackgroundManager.BackgroundType.BattleScreen);
                    Paused = false;
                }
