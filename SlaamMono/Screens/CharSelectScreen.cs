...
namespace Slaam
{
    public class CharSelectScreen : Screen
    {
        ...
        public void Update()
        {
            ...
            if (PeopleIn == 0 && Input.Players[0].PressedAction2 && SelectBoxes[0].CurrentState == CharSelectBoxState.Computer)
                GoBack();
        ...
        public static void LoadAllSkins()
        {
            if (!SkinsLoaded)
            {
                ...
                {
                    SkinTexture[y] = Game1.Content.Load<Texture2D>("content\\skins\\" + Skins[y]);
        ...
        {
            SelectBoxes = new CharSelectBox[Input.Players.Length];

            for(int x = 0; x < Input.Players.Length; x++)
