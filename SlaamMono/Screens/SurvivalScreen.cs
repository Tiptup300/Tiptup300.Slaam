...
namespace Slaam
{
    class SurvivalScreen : GameScreen
    {
        ...
        public override void SetupTheBoard(string BoardLoc)
        {
            ...
            Characters.Add(new Character(Game1.Content.Load<Texture2D>("content\\skins\\" + SetupChars[0].SkinLocation) /*Texture2D.FromFile(Game1.Graphics.GraphicsDevice, SetupChars[0].SkinLocation)*/, SetupChars[0].CharProfile, new Vector2(-100, -100), Input.Players[0], Color.White, 0));
        ...
        private void AddNewBot()
        {
            Characters.Add(new BotPlayer(
                Game1.Content.Load<Texture2D>("content\\skins\\" + CharSelectScreen.ReturnRandSkin())//Texture2D.FromFile(Game1.Graphics.GraphicsDevice, CharSelectScreen.Instance.ReturnRandSkin())
