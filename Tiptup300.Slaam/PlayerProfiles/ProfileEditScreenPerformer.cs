   ...
   public IState Perform(ProfileEditScreenState state)
   {
      ...
      else if (state.CurrentMenu.Value == 1)
      {
         if (state.WaitingForQwerty)
         {
            ...
            if (qwertyStringBack != "")
            {
               ProfileManager.Instance.PlayableProfiles[state.EditingProfile].Name = qwertyStringBack;
         ...
         else
         {
            ...
            if (_inputService.GetPlayers()[0].PressedAction)
            {
               ...
               {
                  ProfileManager.Instance.RemovePlayer(ProfileManager.Instance.PlayableProfiles.GetRealIndex(state.EditingProfile));
               ...
               {
                  string qwertyString = ProfileManager.Instance.PlayableProfiles[state.EditingProfile].Name;

                  state.WaitingForQwerty = true;
               }
               else if (state.SubMenu.Items[state.CurrentMenuChoice.Value].Details[1] == "clr")
               {
                  ProfileManager.Instance.PlayableProfiles[state.EditingProfile].TotalDeaths = 0;
                  ProfileManager.Instance.PlayableProfiles[state.EditingProfile].TotalGames = 0;
                  ProfileManager.Instance.PlayableProfiles[state.EditingProfile].TotalKills = 0;
                  ProfileManager.Instance.PlayableProfiles[state.EditingProfile].TotalPowerups = 0;
                  ProfileManager.Instance.PlayableProfiles[state.EditingProfile].BestGame = TimeSpan.Zero;
                  ProfileManager.Instance.SaveProfiles();
                  state.EditingProfile = -1;
                  state.CurrentMenu.Value = 0;
                  setupMainMenu(state);
