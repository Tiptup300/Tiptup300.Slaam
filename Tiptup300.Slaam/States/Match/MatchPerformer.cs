...
namespace SlaamMono.Gameplay
{
    public class MatchPerformer : IPerformer<MatchState>
    {
        ...
        private void survival_SetupTheBoard(string BoardLoc, MatchState state)
        {
            ...
            state.Characters.Add(new CharacterActor(SlaamGame.Content.Load<Texture2D>("content\\skins\\" + state.SetupCharacters[0].SkinLocation), state.SetupCharacters[0].CharacterProfileIndex, new Vector2(-100, -100), _inputService.GetPlayers()[0], Color.White, 0, ServiceLocator.GetService<IResources>(), _frameTimeService, state.CurrentMatchSettings));
            state.Scoreboards.Add(
                _gameScreenScoreBoardResolver.Resolve(
                    new MatchScoreboardRequest(
                        new Vector2(-250, 10),
